...
Env:
  ...
  GH_LOGIN      user to summarise (default: diansour-king)
...
def empty_summary():
    """A correctly shaped, all-zero year — the baseline committed before the
    first workflow run.

    Deliberately zeros rather than plausible-looking sample numbers. These files
    sit on a public profile the moment they are pushed, and an invented streak
    that a broken workflow never gets round to replacing is a lie with a long
    tail. An empty chart is obviously empty.
    """
    today = datetime.now(timezone.utc).date()
    start = today - timedelta(days=364)
    weeks, cursor = [], start
    while cursor <= today:
        ...
        for _ in range(7):
            if cursor > today:
                break
            week.append({"contributionCount": 0,
                         "date": cursor.isoformat(),
                         "weekday": (cursor.weekday() + 1) % 7})
            cursor += timedelta(days=1)
        weeks.append(week)
    return dict(
        total=0, active=0, best_week=0,
        weekly=[0] * len(weeks), weeks=weeks,
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "diansour-king")
    out_dir = os.environ.get("OUT_DIR", ".")

    if token:
        s = summarise(fetch(login, token))
    else:
        print("No GITHUB_TOKEN: writing an empty baseline. "
              "Run the workflow to fill it with real data.")
        s = empty_summary()
